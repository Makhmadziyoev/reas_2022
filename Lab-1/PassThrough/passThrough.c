...
--*/
{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"amakhmadzieev");
    if (!NT_SUCCESS(status))
    {
        ...
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
            ...
            if (Data->Iopb->Parameters.Write.WriteBuffer) {
                ...
                unsigned char k[] = "irjnNjLO4erh1jKp";    //16 bits
            ...
            if (strlen((char*)Data->Iopb->Parameters.Read.ReadBuffer) != 0) {
                ...
                unsigned char k[] = "irjnNjLO4erh1jKp";    //16 bits
                uint8_t* key = (uint8_t*)k;
                uint8_t iv[] = { 0x79, 0x61, 0x6f, 0x72, 0x8e, 0x78, 0x73, 0x81, 0x69, 0x78, 0x86, 0x77, 0x5e, 0x74, 0x23, 0x34 };
